...
function addUser() {
    ...
    var _url = "http://localhost:8090/SSM_dmm_war/user/showUser"; //按照id显示用户
    $.ajax({
        url: _url,
        type: "POST",
        data: user,
...
//    $.ajax(...)中的内容我们理解，是从abc.php中返回一些值，这些值以json 格式返回，后面的.done()表示$.ajax()如果执行成功
//    ，则执行.done()，funtion(e) 中的e 为返回结果，如果$.ajax()执行发生错误，则.done()不执行。
}
